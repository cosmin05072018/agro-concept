...
<script>
    $(document).ready(function() {
        ('input[type="text"][data-convert-comma]').on('input', function() {
            var inputId = $(this).attr('id');
            var value = $(this).val();

            // Verificăm dacă valoarea conține deja o virgulă sau un punct
            if (value.indexOf(',') !== -1 || value.indexOf('.') !== -1) {
                // Dacă valoarea conține deja o virgulă sau un punct, nu facem nimic
                return;
            }

            // Înlocuim virgulele cu puncte
            value = value.replace(',', '.');
            $(this).val(value);
        });
